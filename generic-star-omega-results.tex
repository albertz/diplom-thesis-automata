...
\subsection{$\Lang^\omega_{\text{Muller}} = \BC \lim \Lang(R)$}
...
First, we show "$\subseteq$", i.e. let $L^\omega \in \BC \lim \Lang(R) \cap \ext \Lang(reg)$. Because $L^\omega \in \ext \Lang(reg)$, there is an E-automaton $\A^E$ which accepts $L^\omega$. We can assume that $\A^E$ is deterministic (with \ref{gen:e-determinism}).
