...
$\tilde L_1 := \ext(L_a) \cap -\ext(L_b)$. Then $\tilde L_1 \not\in \ext \cup \dext \Lang$ but $\tilde L_1 \in \BC \ext \Lang$. (Lemma \ref{P:reg-star})

$\tilde L_2 := \lim(L_a) \cap -\lim(L_b)$. Then $\tilde L_2 \not\in \lim \cup \dlim \Lang$ but $\tilde L_2 \in \BC \lim \Lang$. (Lemma \ref{P:reg-star})
\end{proof}
\end{example}
...
Let $\Lang$ be closed under negation and under alphabet permutation. Let $\Set{a,b,c} \subseteq \Sigma$. Let there be $L_a \in \Lang$ with $\tilde L_a := \ext L_a \not\in \dext \Langreg$.  Let $\Set{a}$ be the \emph{non-invariant alphabet set of $L_a$} and let $L_a$ be \emph{$\Set{b,c}$-invariant}. Then
\[ \ext \cup \dext \Lang \subsetneqq \BC \ext \Lang . \]
\begin{proof}
Define the alphabet permutation $\sigma := \Set{a \mapsto b, b \mapsto a, x \mapsto x \text{ for any } x \in \Sigma-\Set{a,b}}$. $L_a$ is an alphabet permutation non-invariant language because $\sigma(L) \neq L$. Define $L_b := \sigma(L_a)$. Because of the alphabet permutation closure, $L_b \in \Lang$ and $\tilde L_b := \ext L_b \not\in \dext \Langreg$. I.e., $-\tilde L_b = \dext (-L_b) \not\in \ext \Langreg$.

Then, $L_\omega := \tilde L_a \cap -\tilde L_b \in \BC \ext \Lang$.
