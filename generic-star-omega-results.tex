...
is not always the case. We will give an alternative characterization of this property (\cref{gen:postfix-loop-det=limAndBclim}). We will also see that if this holds, we also get
...
Non-postfix-loop-determinism gives us another nice property:
\begin{lemma}
\label{gen:non-postfix-loop-det->extInLim}
Let $\Lang(R)$ be not \emph{postfix-loop-deterministic}. Then
...
Note that non-postfix-loop-determinism doesn't give us closure under suffix-independence:
\begin{example}
There is a congruence relation $R$ such that $\Lang(R)$ is not postfix-loop-deterministic and not closed under suffix-independence.
\begin{proof}
...
\[
  \begin{tikzpicture}[%
    ...
    (2) edge [loop right] node {$a$} ()
    (1) edge [bend right] node {$b$} (3)
    (3) edge [loop right] node {$a,b$} ()
    ;
  \end{tikzpicture}
\]
The SCC $\Set{1,2}$ in the $R$-automaton (re-using the states from the transition graph above) has the loops $\Set{1,2}$ and $\Set{2}$. The SCC $\Set{3}$ has only itself as its loops. Thus, $\Lang(R)$ is not postfix-loop-deterministic.

However, $L_1 := a^+(ba^+)^* \in \Lang(R)$ (all words accepted by state $1$) but $L_1\Sigma^* = a\Sigma^* \not\in \Lang(R)$. I.e. $\Lang(R)$ is not closed under suffix-independence.

Note that in this example, $\ext L_1 = a\Sigma^\omega = \lim(L_1 \cup L_3)$ with $L_3 := (a^+ b)^* b \Sigma^*$ (all words accepted by state $3$). I.e., obviously, $L_1 \cup L_3 \in \Lang(R)$. This is also what we expect from \cref{gen:non-postfix-loop-det->extInLim}, i.e.
\[ \ext \Lang(R) \subseteq \lim \Lang(R) . \]
\end{proof}
\end{example}
...
With \cref{gen:non-postfix-loop-det->extInLim}, we have
