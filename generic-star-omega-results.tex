...
\begin{mydef}
The $L_a \in \Lang$ from \cref{gen:extInBCext}, i.e. $\Set{b,c}$-invariant with non-invariant alphabet set $\Set{a}$, is called \defword{$\Lang$-$\ext$-$\dext$-seperating} if $\ext L_a \not\in \dext \Langreg$. $L_a$ is called \defword{$\Lang$-$\lim$-$\dlim$-seperating} if $\lim L_a \not\in \dlim \Langreg$.
\end{mydef}

\

The \defword{Staiger-Wagner class} is $\BC \ext \Langreg = \lim \cap \dlim \Langreg$. We are interested wether we have the same equality for other language classes $\Lang$. We will first restate the known result for $\Langreg$ and then study the general case.
