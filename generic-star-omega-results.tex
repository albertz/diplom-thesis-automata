...
Look at the SCC $S$ in $\A^M$ where there are final sets $F$ in $\A^M$ with $F \cap S \neq \emptyset$. Let $q \in S$. Let $\F_q$ be the subset of the final state sets of $\A^M$ where $q \in F$ for all $F \in \F_q$. Because those are all loops, we also have $F \subseteq S$ and $\F_q \subseteq 2^S$. Let $\mathcal S_q \subseteq 2^S$ be the set of all possible loops including $q$ in $S$.

1. We want to show that $\F_q \neq \emptyset$. Otherwise, $\A^M$ would only accept words that loop in $S$ but don't visit $q$ infinitely often. We cannot express that negative condition in Büchi.

2. We want to show that $\F_q = \mathcal S_q$.

Case 2: $\F_q = \emptyset$. We want to show that
