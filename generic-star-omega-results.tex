...
\item $\BC \lim \Lang(R) \cap \ext \Langreg \subseteq \ext \Lang(R)$ (\cref{gen:R-bclim-cap-ext})
\end{itemize}
...
\[ \BC \lim \Lang(R) \cap \ext \Langreg \subseteq \ext \Lang(R) \]

\begin{proof}
Let $L^\omega \in \BC \lim \Lang(R) \cap \ext \Langreg$. Because $L^\omega \in \ext \Langreg$, there is an E-automaton $\A^E$ which accepts $L^\omega$. We can assume that $\A^E$ is deterministic (with \cref{gen:e-determinism}).
...
In fact, we actually have shown $\BC \lim \Lang \cap \ext \Langreg \subseteq \ext \Lang$ for any $\Lang \subseteq \Langreg$.

For the equality, we need $\ext \Lang(R) \subseteq \BC \lim \Lang(R)$.
...
However, $L_2 := a(\Sigma a)^* \in \Lang(R)$ (all words accepted by state $2$) but $L_2\Sigma^* = a\Sigma^* \not\in \Lang(R)$. I.e. $\Lang(R)$ is not closed under suffix-independence.

Esp. $\ext L_2 = a \Sigma^\omega \not\in \BC \lim \Lang(R)$.
