...
\begin{lemma}
Let $\Lang$ be a $*$-language class which is \emph{closed under acceptance component variation}. Then
\[ \BC \lim \Lang \cap \lim \Langreg = \lim \Lang . \]
\begin{proof}
We have $\lim \Lang \subseteq \lim \Langreg$ and $\lim \Lang \subseteq \BC \lim \Lang$. Thus, "$\supseteq$" is shown.

Now, we show "$\subseteq$". Let $L_\omega \in \BC \lim \Lang \cap \lim \Langreg$. Let $\A_M$ be a deterministic Muller automaton accepting $L_\omega$ and $\A_B$ a deterministic Büchi automaton accepting $L_\omega$. Construct the canonical product automaton $\A_\times$. Whereever we have a final state in $\A_B$, let this be a final state in $\A_\times$. Then, $\alpha$ is Büchi-accepted by $\A_B$ iff it is Büchi-accepted by $\A_\times$. I.e. $\A_\times$ Büchi-accepts exactly the language $L_\omega$.

TODO: Is $L^*(\A_\times) \in \Lang$? \emph{acceptance component variation closure} is not enough...
%Because of the \emph{acceptance component variation closure}, we also have $L^*(\A_\times) \in \Lang$.
