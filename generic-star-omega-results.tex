...
Case 1: $\F_q = \emptyset$. %We want to show that there is no final state set $F$ in $\A^M$ with $F \cap S \neq \emptyset$ in that case. Otherwise, $\A^M$ would only accept words that loop in $S$ but don't visit $q$ infinitely often. We cannot express that negative condition in Büchi.
...
We show the assumption by contradiction: Let $S \in \mathcal S_q$ with $S \not\in \F_q$. Let $F \in \F_q$ with $F \subseteq S$ or $F \cup S \in \F_q$ (TODO: does that exists?!).
