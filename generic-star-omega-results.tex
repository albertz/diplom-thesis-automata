...
\begin{lemma}
Let $\Lang$ be closed under negation and under alphabet permutation. Let $\Set{a,b,c} \subseteq \Sigma$. Let there be $L \in \Lang$ with $\tilde L := \ext L \not\in \dext \Langreg$.  Let $\Set{a}$ be the \emph{non-invariant alphabet set of $L$} and let $L$ be \emph{$\Set{b,c}$-invariant}. Then
\[ \ext \cup \dext \Lang \subsetneqq \BC \ext \Lang . \]
\begin{proof}
Define $\sigma := \Set{a \mapsto b, b \mapsto a, x \mapsto x \text{ for any } x \in \Sigma-\Set{a,b}}$. $L$ is an alphabet permutation non-invariant language because $L_\sigma \neq L$. Because of the alphabet permutation closure, $L_\sigma \in \Lang$ and $\tilde L_\sigma := \ext L_\sigma \not\in \dext \Langreg$. I.e., $-\tilde L_\sigma = \dext (-L_\Sigma) \not\in \ext \Langreg$.

Then, $L_\omega := \tilde L \cap -\tilde L_\sigma \in \BC \ext \Lang$.
