...
\end{itemize}
Then, a word $w \in \Sigma^*$ satisfies the formular $\phi$ when we set $X = \Set{0,1,2,\dots,\left|w\right|-1}$ and $Q_a n := (w[n] = a)$ and the formular evaluates to a true sentence. If that is the case, we write
\[ w \models \phi , \]
otherwise
\[ w \not\models \phi . \]
The interpretation of such formulas over infinite words is straight-forward: A word $\alpha \in \Sigma^\omega$ satisfies $\phi$ when we set $X = \N_0$ and $Q_a n := (\alpha[n] = a)$ and the formular evaluates to a true sentence.
