...
\item \defword{$\Lang$ closed under alphabet permutation}: For all permutations $\sigma : \Sigma \rightarrow \Sigma$ and $L \in \Lang$, we have $L_\sigma := \Set{\sigma(w)}{w \in L} \in \Lang$
\end{enumerate}
\end{frame}
...
where $w|_M$ is the word $w$ with all letters outside $M$ removed.
