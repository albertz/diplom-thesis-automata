...
Examples: $\Langreg$, $\Lang(\mathtext{starfree})$, $\Lang(\PT_n)$ (Lemma 4.10), $\Lang(\LT)$ \\
...
Examples:
\begin{itemize}
\item $\Langreg$, $\Lang(\mathtext{starfree})$, $\Lang(\PT_n)$, $\Lang(\LT)$ are closed under suffix-independence and negation
\end{itemize}
\end{frame}
...
\BC \lim \Lang .
\end{align*}
For $\Lang := \Lang(\LT)$, via Theorem 3.22, we get
\begin{align*}
& \ext \cap \dext \Lang \subsetneqq
\ext \cup \dext \Lang \subsetneqq
\BC \ext \Lang = \\
