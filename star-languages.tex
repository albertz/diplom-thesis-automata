...
\[ \Lang(\PT) := \bigcup_{n\in\N} \Lang(\simeq_{\PT_n}) . \]

We also have the characterization
\[ \Lang(\PT) = \BC \bigcup_{n\in\N_0,w \in \Sigma^n} \Set{\Sigma^* \cdot w[1] \cdot \Sigma^* \cdots \Sigma^* \cdot w[n] \cdot \Sigma^*} . \]
...
\[ \Lang(\LT) := \bigcup_{n\in\N} \Lang(\simeq_{\text{LT}_n}) . \]
